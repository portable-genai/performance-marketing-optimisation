...
export const TWO_STATE_READS_WITH_A_REASON = {
  NEXT_PUBLIC_FRAME_ANCESTORS:
    "read as a LITERAL property access in proxy.ts and immediately handed to lib/csp.mjs, which " +
    "resolves the three states itself. The literal read is mandatory rather than stylistic: " +
    "Next substitutes NEXT_PUBLIC_* at build time only where it can see the property name in " +
    "the source, so a dynamic read inside the imported module resolves to undefined in the edge " +
    "runtime and would collapse the framing policy to its default without saying so.",
  NEXT_PUBLIC_API_BASE:
    "the same literal-read requirement, for the same reason and to the same module. The three " +
    "states are decided in lib/api.ts and lib/csp.mjs, which share one default so the CSP's " +
    "connect-src permits exactly what the client fetches.",
