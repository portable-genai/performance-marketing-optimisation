...
``tests/unit/test_gcp_adapters_import_safe.py`` covers them by walking a HAND-WRITTEN list of
managed modules and asserting ``google`` never reached ``sys.modules``. That list omits
bindings the settings file really has (``identity`` and the review router), ``sys.modules`` is
shared with everything else in the session so its verdict depends on what imported what first,
and it says nothing about the ``platform`` family, which reuses several of those same modules,
or about the ``vertexai`` root.

Proved rather than asserted: hoisting ``from google.cloud import logging`` to module scope in
``adapters/gcp/iap_identity.py`` left that guard GREEN when run on its own, while the ``gcp``
and ``platform`` cases here went red. Constructing every profile under the same prohibition, in
a fresh interpreter, is what makes the detection deliberate instead of incidental.
