...
GOLDEN = _REPO_ROOT / "eval" / "datasets" / "golden_accounts.jsonl"
...
def _metrics_block() -> list[str]:
    ...
    lines += ["", f"Scored over {len(cases)} golden accounts.", ""]
    return lines
...
def _exercised_block() -> list[str]:
    ...
    cases = load_jsonl(GOLDEN)
    touchpoints = run_eval.journey_touchpoints()
    channels = sum(len(names) for names in touchpoints.values())
    return [
        BLOCKS[1],
        "",
        f"- **{len(cases)} golden accounts** in `eval/datasets/golden_accounts.jsonl`, each with",
        "  the maker-checker expectation a reviewer assigned. The oracle is the dataset's, never",
        "  a re-read of the report's own flag.",
        f"- **{channels} touchpoint channels across {len(touchpoints)} accounts**, read from the",
