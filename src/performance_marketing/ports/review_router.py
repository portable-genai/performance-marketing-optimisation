"""ReviewRouterPort: the boundary that routes an escalated performance report to
...
class ReviewRouterPort(Protocol):
    def route(self, report: PerformanceReport, *, maker: str, tenant: str = "") -> None:
        """Route an escalated report to human-review-console for human review (idempotent per report
        is ideal).
        """
        ...
