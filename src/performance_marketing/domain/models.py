...
class LlmRequest:
    ...
    #: Sampling is decided per call site (owner decision, 2026-09-23). ``None`` sends no
    #: temperature at all, which is what drafting and narration want and the only spelling some
    #: models accept; a call site whose output is extracted, classified, scored or compared pins
    #: ``0.0`` explicitly.
